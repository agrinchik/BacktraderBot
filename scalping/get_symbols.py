...
FUTURE_VOLUME_FILTER_USDT_VAL = 1000000
SPOT_VOLUME_FILTER_USDT_VAL = 1000000
...
class GetSymbols(object):
    ...
    def run(self):
        ...
        quoteasset = args.quoteasset.upper()

        if args.future:
            symbol_info_api_url = FUTURE_SYMBOL_INFO_URL
            filename = "symbols_future_{}.txt".format(quoteasset.lower())
            filter_val = FUTURE_VOLUME_FILTER_USDT_VAL
        else:
            symbol_info_api_url = SPOT_SYMBOL_INFO_URL
            filename = "symbols_spot_{}.txt".format(quoteasset.lower())
